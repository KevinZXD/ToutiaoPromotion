...
public GetTopNews(String limit, String title) {
	super();
	this.limit = limit;
	this.title = title;
}
...
public void getTopNews() throws IOException {
    ...
    String sql="select * from tb_news limit "+limit+" ,15";
     ret  =  DBUtils.querybysql(sql);
	String[] column=new String[]{" ","id","title","url","content","image_url"};
	try {
		int len=ret.getMetaData().getColumnCount();
		if(len!=0){
			 jsonstring = GetResultSet.resultSetToJsonbyColumn(ret, column);
		}else{
			System.out.println("新闻检索出错");
		}
